...
const statusMeta: Record<string, { color: string; bg: string; dot: string; label: string }> = {
  ...
  invalid:        { color: "text-slate-500",   bg: "bg-slate-500/10",   dot: "bg-slate-500",   label: "Invalid" },
  new:            { color: "text-violet-400",  bg: "bg-violet-400/10",  dot: "bg-violet-400",  label: "New" },
};

export function NotifyStatusBadge({ status }: { status: string }) {
...
export function NotifyStatusForm({
  notifyId,
  currentStatus,
}: {
  notifyId: string;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await fetch(`/admin/notify/${notifyId}`, { method: "POST", body: formData });
      setSaved(true);
