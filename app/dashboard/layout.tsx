...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-[76px] left-0 h-[calc(100vh-76px)] w-64 bg-white shadow-md transform transition-transform duration-300 ease-in-out z-40
      ...
      >
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <Button variant="ghost" className="lg:hidden" onClick={toggleMenu} aria-label="Close menu">
            <X className="h-6 w-6" />
          </Button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="lg:hidden fixed top-0 left-0 right-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-20 h-[76px]">
          ...
          <Button variant="outline" onClick={toggleMenu} aria-label="Open menu" className="p-2">
