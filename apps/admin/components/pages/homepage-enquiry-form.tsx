...
type HomepageEnquiry = {
  id: string;
  name: string;
  mobile_number: string;
  preferred_destination: string;
  created_at: string;
  contacted?: boolean; // for status toggle
};

export function HomepageEnquiryFormPage() {
  const [searchTerm, setSearchTerm] = useState("");
  const [filterDate, setFilterDate] = useState<
    "all" | "today" | "yesterday" | "week" | "month"
  >("all");
  const [data, setData] = useState<HomepageEnquiry[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        console.log("🔍 Fetching from table: homepage_inquiries");
        ...
        setError(err.message || "Failed to fetch homepage enquiries");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleStatusToggle = async (id: string, currentStatus?: boolean) => {
    ...
    if (error) {
      ...
      setData((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, contacted: newStatus } : item
        )
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
      <Card className="border-border">
        <CardHeader>
          <CardTitle>Homepage Enquiry Form Submissions</CardTitle>
          <CardDescription>Manage all homepage inquiries</CardDescription>
        </CardHeader>
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="border-b border-border bg-muted/50">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium w-[15%]">Mobile</th>
                  <th className="text-left px-4 py-3 font-medium w-[25%]">
                ...
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-pre-wrap break-words capitalize">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleStatusToggle(item.id, item.contacted)
                            }
                          >
