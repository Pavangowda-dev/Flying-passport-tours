...
export function TourBookingFormPage() {
  const [searchTerm, setSearchTerm] = useState("");
  const [filterDate, setFilterDate] = useState<
    "all" | "today" | "yesterday" | "week" | "month"
  >("all");
  const [data, setData] = useState<TourBooking[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        console.log("🔍 Fetching from table: group_tour_bookings");
        ...
        setError(err.message || "Failed to fetch tour booking data");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleVerifyToggle = async (id: string, currentStatus?: boolean) => {
    ...
    if (error) {
      ...
      setData((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, verified: newStatus } : item
        )
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
      <Card className="border-border">
        <CardHeader>
          <CardTitle>Tour Booking Form Submissions</CardTitle>
          <CardDescription>Manage all group tour bookings</CardDescription>
        </CardHeader>
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="border-b border-border bg-muted/50">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium w-[15%]">Contact Type</th>
                  <th className="text-left px-4 py-3 font-medium w-[20%]">Contact Value</th>
                ...
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{item.contact_type}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleVerifyToggle(item.id, item.verified)
                            }
                          >
