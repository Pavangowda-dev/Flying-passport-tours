...
interface TourBookingFormPageProps {
  initialData: TourBooking[]
}

export function TourBookingFormPage({ initialData }: TourBookingFormPageProps) {
  const [searchTerm, setSearchTerm] = useState("")
  const [filterDate, setFilterDate] = useState<"all" | "today" | "yesterday" | "week" | "month">("all")
  const [currentPage, setCurrentPage] = useState(1)
  const [data, setData] = useState<TourBooking[]>(initialData)
  const [selectedItem, setSelectedItem] = useState<TourBooking | null>(null)
  ...
  const handleVerifyToggle = async (id: string, currentStatus?: boolean) => {
    ...
    if (error) {
      ...
      setData(prev =>
        prev.map(item => item.id === id ? { ...item, verified: newStatus } : item)
      )
      setIsUpdating(null)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    if (!supabase || !confirm("Are you sure you want to delete this booking? This action cannot be undone.")) return
  ...
  const openModal = (item: TourBooking) => {
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-700 mt-1">{stats.verified}</p>
              </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <CardDescription className="text-sm mt-1 text-gray-600">
                Showing {filteredData.length} of {data.length} total bookings
              </CardDescription>
            </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                            ...
                            <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-white">
                              ...
                              <div className="space-y-6 pt-4 bg-white">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  ...
                                  <div className="space-y-2 md:col-span-2">
                                    <label className="text-sm font-semibold text-[#1f2d28]">Booked On</label>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t bg-white">
                                  ...
                                  >
                                    {item.verified ? "Verified Booking" : "Pending Verification"}
                                  </Badge>
                                  <Button
                                    variant="outline"
                          ...
                          <Button
                            ...
                            title={item.verified ? "Mark as pending" : "Mark as verified"}
                            onClick={() => handleVerifyToggle(item.id, item.verified)}
